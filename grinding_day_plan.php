...
session_start();
$title=trad('grinding_day_plan',$_SESSION["language"]);
$pi=pi();
?>
...
<html lang="fr" class="<?php echo $_SESSION['mode'];?>">
    ...
    <head>
        ...
        <title><?php echo trad('grinding_day_plan',$_SESSION["language"]);?></title>
        <!-- BEGIN: CSS Assets-->
        ...
        <div class="box d-sm-flex mt-8 px-8 py-12">
                    <div class="p-5 border-bottom border-gray-200 dark-border-dark-5">
                        <h4 class="fs-xl fw-medium lh-1 mt-3">
                            <?php echo trad('grinding',$_SESSION["language"]);?>
                        </h4>
                        <div class="overflow-x-auto">
                            <table class="table mt-40">
                                ...
                                <tbody>
                                    <?php
                                    $jobs=select_jobs_by_day_and_step_id($date, '4');
                                    ?>
                                    <?php
                                    foreach($jobs as $job){
                                        if($job['pt_status'] != "Finished"){?>
                                        
                                            <tr onclick="document.location='do_job.php?plan_id=<?php echo $job['pt_id'];?>&piece=<?php echo $job['pc_id'].'&step=4';?>'" class="plan_table">
                                                <td>
                                                    <?php echo $job['pc_id'];?>
                                                </td>
                                                <td>
