...
session_start();
$title=trad('lining_day_plan',$_SESSION["language"]);
$pi=pi();

$furthest_date=select_furthest_lining_plan_date();
$now=date('Y-m-d');
...
foreach ($days as $day) {
    ...
    if($over_time==[] && ($day_number==1 || $day_number == 7 || $day_without_year=='01-01' || $day_without_year=='05-01' || $day_without_year=='05-08' || $day_without_year=='07-14' || $day_without_year=='08-15' || $day_without_year=='11-01' || $day_without_year=='11-11' || $day_without_year=='12-25')){
        ...
        $verif_occupation=0;
        // jour travaillé
        $default_times=select_operator_default_time_by_sector('2');
        $dispo=0;
        ...
        $absences=select_operator_absence_at_date_and_by_sector($day_string,'2');
    ...
    $times_manuel=select_estimated_workload_at_date_by_step($day_string,'2');
    $times_mechanical=select_estimated_workload_at_date_by_step($day_string,'6');

    
    $occupation_manuel=null;
    foreach($times_manuel as $time_manuel){
        $occupation_manuel=$occupation_manuel+$time_manuel['nb_minute'];
    }

    $occupation_mechanical=null;
    foreach($times_mechanical as $time_mechanical){
        $occupation_mechanical=$occupation_mechanical+$time_mechanical['nb_minute'];
    }

    $occupation=$occupation_mechanical+$occupation_manuel;
...
<body class="main">
    ...
    ?>  
    <h4 class="fs-xl fw-medium lh-1 mt-3 mb-6"><?php echo trad('lining_workshop',$_SESSION['language']);?></h4>
    <div class="content">
        <?php
        $v=0;
        foreach($days as $day){
                        ...
                        <div class="intro-y box">
                            ...
                            <div id="boxed-accordion" class="p-5">
                                <div class="preview">
                                    ...
                                    <?php
                                        //selection des commandes du jour pour le garnissage
                                        $jobs=select_jobs_by_day_and_step_id($day, '2');
                                        foreach($jobs as $job){
