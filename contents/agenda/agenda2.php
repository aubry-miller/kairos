	...
	$recipes=select_conges_acceptes_par_date($date);
		...
		foreach($recipes as $recipe){
			...
			if($recipe['conges_hours_day']==''){
				echo '<span style="color:#003366;"><a onclick="ouvrirFenetre(\'visu_demande_conges.php?id='.$recipe['conges_id'].'\')">'.$recipe['prenom'].' '.$recipe['nom']."</a></span><br>";
			} else if ($recipe['conges_hours_start']>12 && $recipe['conges_hours_end']>=12){
				echo '<span style="color:#003366;"><a onclick="ouvrirFenetre(\'visu_demande_conges.php?id='.$recipe['conges_id'].'\')">'.$recipe['prenom'].' '.$recipe['nom']."</a></span>(AM)<br>";
			} else if ($recipe['conges_hours_start']<=12 && $recipe['conges_hours_end']<12){
				echo '<span style="color:#003366;"><a onclick="ouvrirFenetre(\'visu_demande_conges.php?id='.$recipe['conges_id'].'\')">'.$recipe['prenom'].' '.$recipe['nom']."</a></span>(PM)<br>";
			} else{
				echo '<span style="color:#003366;"><a onclick="ouvrirFenetre(\'visu_demande_conges.php?id='.$recipe['conges_id'].'\')">'.$recipe['prenom'].' '.$recipe['nom']."</a></span>(Partiel)<br>";
			}
		}
		$demandeeRecipes=select_conges_demandes_par_date($date);
		foreach($demandeeRecipes as $demandeeRecipe){
			if($recipe['conges_hours_day']==''){
				echo '<span style="color:red;"><a onclick="ouvrirFenetre(\'visu_demande_conges.php?id='.$recipe['conges_id'].'\')">'.$demandeeRecipe['prenom'].' '.$demandeeRecipe['nom']."</a></span><br>";
			} else if ($recipe['conges_hours_start']>12 && $recipe['conges_hours_end']>=12){
				echo '<span style="color:red;"><a onclick="ouvrirFenetre(\'visu_demande_conges.php?id='.$recipe['conges_id'].'\')">'.$demandeeRecipe['prenom'].' '.$demandeeRecipe['nom']."</a>(AM)</span><br>";
			} else if ($recipe['conges_hours_start']<=12 && $recipe['conges_hours_end']<12){
				echo '<span style="color:red;"><a onclick="ouvrirFenetre(\'visu_demande_conges.php?id='.$recipe['conges_id'].'\')">'.$demandeeRecipe['prenom'].' '.$demandeeRecipe['nom']."</a>(PM)</span><br>";
			} else{
				echo '<span style="color:red;"><a onclick="ouvrirFenetre(\'visu_demande_conges.php?id='.$recipe['conges_id'].'\')">'.$demandeeRecipe['prenom'].' '.$demandeeRecipe['nom']."</a>(Partiel)</span><br>";
			}
		}
	echo '</div></div>';
	echo "</td>";
